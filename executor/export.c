...
/*   Updated: 2023/08/07 22:46:59 by cmansey          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	update_existing_variable(char **existing_var, char *new_value)
{
	...
	var_name_length = strchr(*existing_var, '=') - *existing_var;
	...
	strncpy(new_var, *existing_var, var_name_length);
	new_var[var_name_length] = '\0';
	strcat(new_var, "=");
	strcat(new_var, new_value);
	free(*existing_var);
...
void	add_new_variable(char ***environ, char *var)
{
	...
	new_environ[env_size] = strdup(var);
...
void	replace_variable(char *var, char ***environ)
{
	...
	{
		if (strncmp(*existing_var, var, var_name_length) == 0)
		{
